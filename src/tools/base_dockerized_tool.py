...
class BaseDockerizedTool(abc.ABC):
    ...
    async def _postprocess(self, *, _output: str, token: uuid.UUID, **kwargs) -> tuple[str, list[str]]:
        ...
        """
        if file_names_to_push := await self._get_output_files(token=token, _output=_output, **kwargs):
            return _output, [
                file
    ...
    async def _get_output_files(self, *, _output: str, **kwargs) -> list[str]:
        """
        Get the output files generated by the dockerized tool.

        Args:
            _output (str): The output string from the tool.
            **kwargs: Input data for the tool.
        Returns:
            list[str]: List of output files generated by the tool.
        """
        return []
